...
CUSTOM_IMAGES = './data/vdp_small/'
VG_IMAGES = './data/vdp_small/'
RCNN_CHECKPOINT_FN = path('faster_rcnn_500k.h5')
IM_DATA_FN = stanford_path('image_data.json')
CUSTOM_DATA_FN = stanford_path('custom_data.json')
VG_SGG_FN = stanford_path('VG-SGG.h5')
VG_SGG_DICT_FN = stanford_path('VG-SGG-dicts.json')
CUSTOM_SGG_FN = stanford_path('VG-SGG.h5')
CUSTOM_SGG_DICT_FN = stanford_path('VG-SGG-dicts.json')
...
# =============================================================================
# =============================================================================
def gen_custom_data_json(dump_loc=stanford_path('custom_data.json')):
    from glob import glob
    from PIL import Image
    import json
    img_paths = glob(CUSTOM_IMAGES + "*")
    img_props = []
    ...
    with open(dump_loc, 'w') as f:
        json.dump(img_props, f)


if not os.path.exists(CUSTOM_DATA_FN):
    print("Making custom img_data thingie...")
    gen_custom_data_json(CUSTOM_DATA_FN)
