...
if [ $1 == "0" ]; then
    ...
    CUDA_VISIBLE_DEVICES=0 python -O models/eval_rels.py -m sgcls -model motifnet -order leftright -nl_obj 2 -nl_edge 4 -b 6 -clip 5 \
        -p 100 -hidden_dim 512 -pooling_dim 4096 -lr 1e-3 -ngpu 1 -test -ckpt checkpoints/vgrel-motifnet-sgcls.tar -nepoch 1 -use_bias -limit_vision -cache motifnet_sgcls
    CUDA_VISIBLE_DEVICES=0 python -O models/eval_rels.py -m predcls -model motifnet -order leftright -nl_obj 2 -nl_edge 4 -b 6 -clip 5 \
        -p 100 -hidden_dim 512 -pooling_dim 4096 -lr 1e-3 -ngpu 1 -test -ckpt ./checkpoints/vgrel-motifnet-sgcls.tar -nepoch 1 -use_bias -limit_vision -cache motifnet_predcls
fi
